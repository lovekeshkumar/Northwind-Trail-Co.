 ...
 * analytics.js
 * ------------------------------------------------------------------
 * Thin wrapper around the real Adobe runtime.
 *
 * VisitorAPI.js and AppMeasurement.js are loaded up front in index.html.
 * This file does not load/unload Adobe. It just:
 *   - initializes the real s object,
 *   - applies consent to adobe.optIn,
 *   - fires tracking calls when Analytics consent exists,
 *   - persists manual browser-console complete() calls.
 * ------------------------------------------------------------------
 */
...
const AdobeAnalytics = (() => {
  ...
  function currentConsentFromStorage() {
    try {
      const raw = localStorage.getItem('nt_consent_v1');
  ...
  function persistConsent(consent) {
    ...
    try {
      localStorage.setItem('nt_consent_v1', JSON.stringify(record));
    } catch {
      // ignore
    }
